...
#   - Make `department` field refer to a `Department` object, not the department's name
#   - Add `discount()` method
#   - Add `discounted_price_per_item()` method
# - Rename `DepartmentStatsMap` to `OrderTracker`, remove `UserDict` inheritance
#   - Add `_departments_stats_dict` field
#   - Add `register_sale()` method
#   - Add `get_department_stats()` method
#   - Add `products` field
# - Remove `products` parameter from functions taking an `OrderTracker` parameter
# - Add `customer` field to `OrderTracker`
# - Move `result` variable to `OrderTracker` class
# (Refactoring not finished here, will continues in invoice_v9.py)
...
class OrderTracker:
    ...
    _department_stats_dict: dict[str, DepartmentStats] = field(default_factory=dict)
    _result: io.StringIO = field(default_factory=io.StringIO)

    def __post_init__(self):
        self._result.write(f"Order for {self.customer}:\n")
    ...
    def register_sale(self, product, price, quantity):
...
def generate_invoice(order, products):
    ...
    for line_item in order["items"]:
        add_line_for_single_item(order_tracker, line_item)

    add_volume_discount(order_tracker)
    return order_tracker.result


def add_line_for_single_item(order_tracker: OrderTracker, line_item: tuple[str, int]):
    item, quantity = line_item
    product: Product = order_tracker.products[item]
    price = product.total_price(quantity)
    order_tracker.register_sale(product, price, quantity)
    order_tracker._result.write(
        f"  - {item + ':':12} {quantity:4} à {product.price_per_item :3.2f} = ${price:.2f}"
    )
    if product.discount(quantity) > 0:
        order_tracker._result.write(
            f" (discount: ${product.price_per_item * quantity - price:.2f})\n"
        )
    else:
        order_tracker._result.write("\n")


def add_volume_discount(order_tracker: OrderTracker):
    total = 0.0
    volume_discount = 0.0
    for department_name, department in _departments.items():
        department_stats = order_tracker.get_department_stats(department_name)
    ...
    if volume_discount > 0:
        order_tracker._result.write(f"Subtotal:        ${total:.2f}\n")
        order_tracker._result.write(f"Volume discount: ${volume_discount:.2f}\n")
        order_tracker._result.write(f"Total:           ${total - volume_discount:.2f}")
    else:
        order_tracker._result.write(f"Total: ${total:.2f}")
