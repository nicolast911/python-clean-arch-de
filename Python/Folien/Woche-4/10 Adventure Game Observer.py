...
class Player:
    ...
    def note_turn_started(self):
        for observer in self.observers: observer.notify_turn_started(self)  # noqa: E701

    def note_turn_ended(self):
        for observer in self.observers: observer.notify_turn_ended(self)  # noqa: E701

    def note_action_selected(self, action: Action):
        for observer in self.observers: observer.notify_action_selected(self, action)  # noqa: E701
...
# %%
game.attach_observer(CliObserver())
...
# %%
game.attach_observer(DebugGameObserver())
...
class LibraryObserver:
    def on_book_added(self, book: BookInfo):
        pass

    def on_member_added(self, member: MemberInfo):
        pass

    def on_book_borrowed(self, member_id: str, isbn: str):
        pass

    def on_book_returned(self, member_id: str, isbn: str):
        pass
...
# %%
class ConsoleObserver(LibraryObserver):
    def on_book_added(self, book: BookInfo):
        print(
            f"Book added: {book.title} by {', '.join(book.authors)} (ISBN: {book.isbn})"
        )
    ...
    def on_book_borrowed(self, member_id: str, isbn: str):
        print(f"Book borrowed: Member {member_id} borrowed book with ISBN {isbn}")

    def on_book_returned(self, member_id: str, isbn: str):
        print(f"Book returned: Member {member_id} returned book with ISBN {isbn}")
    ...
    def borrow_book(self, member_id: str, isbn: str, book_inventory: BookInventory):
        if book_inventory.is_available(isbn):
            ...
            if isbn not in self.rentals[member_id]:
                ...
                raise ValueError(
                    f"Member {member_id} already has book with ISBN {isbn}"
                )
        else:
...
class Library:
    ...
    member_registry: MemberRegistry = field(default_factory=MemberRegistry)
    rental_registry: RentalRegistry = field(default_factory=RentalRegistry)
    observers: list[LibraryObserver] = field(default_factory=list)

    def add_observer(self, observer: LibraryObserver):
        self.observers.append(observer)

    def notify_book_added(self, book: BookInfo):
        for observer in self.observers:
            observer.on_book_added(book)

    def notify_member_added(self, member: MemberInfo):
        for observer in self.observers:
            observer.on_member_added(member)

    def notify_book_borrowed(self, member_id: str, isbn: str):
        for observer in self.observers:
            observer.on_book_borrowed(member_id, isbn)

    def notify_book_returned(self, member_id: str, isbn: str):
        for observer in self.observers:
            observer.on_book_returned(member_id, isbn)
    ...
    def add_book(self, title: str, authors: list[str], isbn: str, num_copies: int = 1):
        book = BookInfo(title, authors, isbn, num_copies)
        self.book_inventory.add_book(book)
        self.notify_book_added(book)

    def add_member(self, name: str, address: str, member_id: str):
        member = MemberInfo(name, address, member_id)
        self.member_registry.add_member(member)
        self.notify_member_added(member)

    def borrow_book(self, member_id: str, isbn: str):
        try:
            self.rental_registry.borrow_book(member_id, isbn, self.book_inventory)
            self.notify_book_borrowed(member_id, isbn)
        except ValueError as e:
            self.notify_error(str(e))

    def return_book(self, member_id: str, isbn: str):
        self.rental_registry.return_book(member_id, isbn, self.book_inventory)
        self.notify_book_returned(member_id, isbn)
...
library.add_observer(console_observer)
